            ...
            foreach (string entry in peopleAndBalances)
            {
                ...
                for (int i = 1; i < userNameAndBalanceHistory.Length; i++)
                {
                    ...
                    if (balance == highestBalance)
                    {
                        if (holderCount == 2)
                        {
                            highestBalanceHolder += Invariant($" and {currentBalanceHolder}");
                        }
                        if (holderCount == 1)
                        {
                            highestBalanceHolder += Invariant($", {currentBalanceHolder}");
                            holderCount++;
                        }
                    }
            ...
            foreach (string entry in peopleAndBalances)
            {
                ...
                for (int i = 1; i < userHistory.Length - 1; i++)
                {
                    ...
                    if (biggestLoss == nextBalance - balance)
                    {
                        if (holderCount > 1)
                        {
                            biggestLoser += Invariant($" and {currentBalanceHolder}");
                        }
                        if (holderCount == 1)
                        {
                            biggestLoser += Invariant($", {currentBalanceHolder}");
                            holderCount++;
                        }
                    }
        ...
        public static string FindRichestPerson(string[] peopleAndBalances)
        {
            ...
            foreach (string entry in peopleAndBalances)
            {
                ...
                if (highestLastBalance == currentBalanceHolderFinalBalance &&
                    ...
                    holderCount == 2)
                {
                    highestLastBalanceHolder += Invariant($" and {currentBalanceHolder}");
                }
                if (highestLastBalance == currentBalanceHolderFinalBalance &&
                   highestLastBalanceHolder != currentBalanceHolder &&
                   holderCount == 1)
                {
                    highestLastBalanceHolder += Invariant($", {currentBalanceHolder}");
                    holderCount++;
                }
